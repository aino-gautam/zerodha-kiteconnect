...
public class KiteConnect {
	 ...
	 * 
	 * @param apiKey is the api key provided after creating new Kite Connect app on
	 *               developers console.
	 */
	 ...
	 * Initializes KiteSDK with the api key provided for your app.
	 * 
	 * @param apiKey         is the api key provided after creating new Kite Connect
	 ...
	 *                  on developers console.
	 * @param userProxy is the user defined proxy. Can be used only if a user chose
	 *                  to use the proxy.
	 */
	...
	public User generateSession(String requestToken, String apiSecret)
		...
		return new User()
				.parseResponse(kiteRequestHandler.postRequest(routes.get("api.validate"), params, apiKey, accessToken));
	}
	 ...
	 * @throws KiteException is thrown for all Kite trade related errors.
	 */
	public TokenSet renewAccessToken(String refreshToken, String apiSecret)
	 ...
	 * 
	 * @return Map of String and Margin is a map of commodity or equity string and
	 *         funds data.
	 * @throws KiteException is thrown for all Kite trade related errors.
	 * @throws JSONException is thrown when there is exception while parsing
	 *                       response.
	 ...
	 * 
	 * @return MarginCalculationData list contains the total, var, exposure, span
	 *         and other components of the margin required.
	 * @throws KiteException is thrown for all Kite trade related errors.
	 * @throws JSONException is thrown when there is exception while parsing
	 *                       response.
	 * @throws IOException   is thrown when there is connection error.
	...
	public List<MarginCalculationData> getMarginCalculation(List<MarginCalculationParams> params)
		...
		JSONObject response = kiteRequestHandler.postRequestJSON(url, jsonArray, new HashMap<String, Object>(), apiKey,
				accessToken);
	 ...
	 * 
	 * @return CombinedMarginData object, it contains the initial, final and margin
	...
	public CombinedMarginData getCombinedMarginCalculation(List<MarginCalculationParams> params,
			boolean considerPositions, boolean compactMode) throws IOException, KiteException, JSONException {
		String url = routes.get("margin.calculation.basket");
		Map<String, Object> queryParams = new HashMap<String, Object>();

		if (considerPositions)
			queryParams.put("consider_positions", true);
		if (compactMode)
			queryParams.put("mode", "compact");
	...
	/**
	 * A virtual contract provides detailed charges order-wise for brokerage, STT,
	 * stamp duty, exchange transaction charges, SEBI turnover charge, and GST.
	 * 
	 * @return ContractNote list contains the data and charges for all the orders
	 *         passed as input.
	 * @throws KiteException is thrown for all Kite trade related errors.
	 * @throws JSONException is thrown when there is exception while parsing
	 *                       response.
	 ...
	 * Place a GTT.
	 * 
	 * @param gttParams is GTT param which container condition, type, order details.
	 *                  It can contain one or two orders.
	 * @throws IOException   is thrown when there is connection error.
	 * @throws KiteException is thrown for all Kite trade related errors.
	 * @throws JSONException is thrown when there is exception while parsing
	 *                       response.
	 ...
	 * @param exchange        Exchange in which instrument is listed (NSE, BSE, NFO,
	 *                        BFO, CDS, MCX).
	 ...
	 *
	 * Response is array for objects. For example, { instrument_token: '131098372',
	 * exchange_token: '512103', tradingsymbol: 'NIDHGRN', name: 'NIDHI GRANITES',
	 * last_price: '0.0', expiry: '', strike: '0.0', tick_size: '0.05', lot_size:
	 * '1', instrument_type: 'EQ', segment: 'BSE', exchange: 'BSE' }, ...]
	 * 
	 ...
	 * '1', instrument_type: 'EQ', segment: 'BSE', exchange: 'BSE' }, ...]
	 * 
	 * @param exchange Filter instruments based on exchange. exchange can be NSE,
	 *                 BSE, NFO, BFO, CDS, MCX.
	 ...
	 *
	 * @param instruments is the array of tradingsymbol and exchange or instrument
	 *                    token. For example {NSE:NIFTY 50, BSE:SENSEX} or {256265,
	 *                    265}
	 *
	...
	public Map<String, OHLCQuote> getOHLC(String[] instruments) throws KiteException, IOException, JSONException {
		JSONObject resp = kiteRequestHandler.getRequest(routes.get("quote.ohlc"), "i", instruments, apiKey,
				accessToken);
	...
	public Map<String, LTPQuote> getLTP(String[] instruments) throws KiteException, IOException, JSONException {
		JSONObject response = kiteRequestHandler.getRequest(routes.get("quote.ltp"), "i", instruments, apiKey,
				accessToken);
	 ...
	 * @param instruments     is the array of tradingsymbol and exchange or
	 *                        instrument token.
	 ...
	 * 
	 * @param from       "yyyy-mm-dd" for fetching candles between days and
	 *                   "yyyy-mm-dd hh:mm:ss" for fetching candles between
	 *                   timestamps.
	 * @param to         "yyyy-mm-dd" for fetching candles between days and
	 *                   "yyyy-mm-dd hh:mm:ss" for fetching candles between
	 *                   timestamps.
	 * @param continuous set to true for fetching continuous data of expired
	 *                   instruments.
	 * @param interval   can be minute, day, 3minute, 5minute, 10minute, 15minute,
	 *                   30minute, 60minute.
	 * @param token      is instruments token.
	 * @param oi         set to true for fetching open interest data. The default
	 *                   value is 0.
	 * @return HistoricalData object which contains list of historical data termed
	 *         as dataArrayList.
	 * @throws KiteException is thrown for all Kite trade related errors.
	 * @throws IOException   is thrown when there is connection related error.
	 */
	public HistoricalData getHistoricalData(Date from, Date to, String token, String interval, boolean continuous,
			boolean oi) throws KiteException, IOException, JSONException {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Map<String, Object> params = new HashMap<>();
	 ...
	 * @param transactionType BUY or SELL.
	 * @param amount          Amount worth of units to purchase. Not applicable on
	 *                        SELLs.
	 * @param quantity        Quantity to SELL. Not applicable on BUYs. If the
	 *                        holding is less than minimum_redemption_quantity, all
	 *                        the units have to be sold.
	...
	public MFOrder placeMFOrder(String tradingsymbol, String transactionType, double amount, double quantity,
			String tag) throws KiteException, IOException, JSONException {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("tradingsymbol", tradingsymbol);
		params.put("transaction_type", transactionType);
		...
		JSONObject response = kiteRequestHandler.postRequest(routes.get("mutualfunds.orders.place"), params, apiKey,
				accessToken);
	...
	/**
	 * If cancel is successful then api will respond as 200 and send back true else
	 * it will be sent back to user as KiteException.
	 * 
	...
	public MFOrder getMFOrder(String orderId) throws KiteException, IOException, JSONException {
		JSONObject response = kiteRequestHandler
	 ...
	 * @param amount         Amount worth of units to purchase. It should be equal
	 *                       to or greated than minimum_additional_purchase_amount
	 *                       and in multiple of purchase_amount_multiplier in the
	 *                       instrument master.
	 * @param installmentDay If Frequency is monthly, the day of the month (1, 5,
	 *                       10, 15, 20, 25) to trigger the order on.
	 * @param instalments    Number of instalments to trigger. If set to -1,
	 *                       instalments are triggered at fixed intervals until the
	 *                       SIP is cancelled.
	 * @param initialAmount  Amount worth of units to purchase before the SIP
	 *                       starts. Should be equal to or greater than
	 *                       minimum_purchase_amount and in multiple of
	 *                       purchase_amount_multiplier. This is only considered if
	 *                       there have been no prior investments in the target
	 *                       fund.
	...
	public MFSIP placeMFSIP(String tradingsymbol, String frequency, int installmentDay, int instalments,
			int initialAmount, double amount) throws KiteException, IOException, JSONException {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("tradingsymbol", tradingsymbol);
		...
		JSONObject response = kiteRequestHandler.postRequest(routes.get("mutualfunds.sips.place"), params, apiKey,
				accessToken);
	 ...
	 * @param amount      Amount worth of units to purchase. It should be equal to
	 *                    or greated than minimum_additional_purchase_amount and in
	 *                    multiple of purchase_amount_multiplier in the instrument
	 *                    master.
	 * @param day         If Frequency is monthly, the day of the month (1, 5, 10,
	 *                    15, 20, 25) to trigger the order on.
	 * @param instalments Number of instalments to trigger. If set to -1,
	 *                    instalments are triggered at fixed intervals until the SIP
	 *                    is cancelled.
	...
	public boolean modifyMFSIP(String frequency, int day, int instalments, double amount, String status, String sipId)
		...
		kiteRequestHandler.putRequest(routes.get("mutualfunds.sips.modify").replace(":sip_id", sipId), params, apiKey,
				accessToken);
		return true;
	}
	...
	public boolean cancelMFSIP(String sipId) throws KiteException, IOException, JSONException {
		kiteRequestHandler.deleteRequest(routes.get("mutualfunds.sip").replace(":sip_id", sipId),
				new HashMap<String, Object>(), apiKey, accessToken);
		return true;
	}
