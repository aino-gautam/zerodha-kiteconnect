...
public class KiteRequestHandler {
	 ...
	 * @throws JSONException is thrown for parsing errors.
	 */
	public JSONObject postRequestJSON(String url, JSONArray jsonArray, Map<String, Object> queryParams, String apiKey,
			String accessToken) throws IOException, KiteException, JSONException {
	...
	public Request createJsonPostRequest(String url, JSONArray jsonArray, Map<String, Object> queryParams,
		...
		Request request;
		request = queryParams.size() > 0
				? new Request.Builder().url(httpBuilder.build()).header("User-Agent", USER_AGENT)
						.header("X-Kite-Version", "3").header("Authorization", "token " + apiKey + ":" + accessToken)
						.post(body).build()
				: new Request.Builder().url(url).header("User-Agent", USER_AGENT).header("X-Kite-Version", "3")
