...
public class KiteResponseHandler {
	...
	public JSONObject handle(Response response, String body) throws IOException, KiteException, JSONException {
		...
		if (body.length() > 0 && response.header("Content-Type").contains("json")) {
			...
			throw new DataException(
					"Unexpected content type received from server: " + response.header("Content-Type") + " " + body,
					502);
		}
	}
	...
	private KiteException dealWithException(JSONObject jsonObject, int code) throws JSONException {
		...
		case "MarginException":
			return new MarginException(jsonObject.getString("message"), code);

		case "HoldingException":
			return new HoldingException(jsonObject.getString("message"), code);
