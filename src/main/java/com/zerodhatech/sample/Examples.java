...
public class Examples {
	...
	public void placeIcebergOrder(KiteConnect kiteConnect) throws KiteException, IOException {
		/**
		 * Iceberg order:- following is example param for iceberg order with ttl
		 * validity. Minimum number of legs is 2 and maximum number of legs is 10. TTL
		 * validity is always sent as integer wherein number denotes number of minutes
		 * an order can be alive.
		 */
		OrderParams orderParams = new OrderParams();
	...
	/**
	 * Get ongoing auction instruments available for the day and it usually starts
	 * at 2 PM on a trading day.
	 */
	...
	public void getOrders(KiteConnect kiteConnect) throws KiteException, IOException {
		...
		for (int i = 0; i < orders.size(); i++) {
			System.out.println(orders.get(i).tradingSymbol + " " + orders.get(i).orderId + " "
					...
					+ " " + orders.get(i).exchangeTimestamp + " " + orders.get(i).exchangeUpdateTimestamp + " "
					+ orders.get(i).guid);
		}
		// Read iceberg params
		/**
		 * Map<String, Object> meta = orders.get(0).meta; Map icebergObject = (Map)
		 * meta.get("iceberg"); System.out.println(icebergObject.keySet());
		 */
	...
	public void placeGTT(KiteConnect kiteConnect) throws IOException, KiteException {
		...
		/**
		 * Only sell is allowed for OCO or two-leg orders. Single leg orders can be buy
		 * or sell order. Passing a last price is mandatory. A stop-loss order must have
		 * trigger and price below last price and target order must have trigger and
		 * price above last price. Only limit order type and CNC product type is allowed
		 * for now.
		 */
	...
	public void getHistoricalData(KiteConnect kiteConnect) throws KiteException, IOException {
		 ...
		 * interval, continuous (for expired F&O contracts), oi (open interest) returns
		 * historical data object which will have list of historical data inside the
		 * object.
		 */
